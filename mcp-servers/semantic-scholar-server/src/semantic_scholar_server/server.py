...
from rwa_result_store import (
    execute_search_script as _execute_search_script,
)
from rwa_result_store import (
...
@mcp.tool()
async def draft_semantic_scholar_search(
    query: str,
    project_path: str,
    year_range: str | None = None,
    fields_of_study: str | None = None,
    ...
    """Draft a reproducible Semantic Scholar search script for user review.
    ...
    Args:
        ...
        open_access_only: If True, only open access papers.
        limit: Maximum results (default 20, max 100).
        project_path: Project directory path for script storage.

    Returns:
        Dictionary with script_path and script_content for review.
    """
    limit = min(limit, 100)
    ...
    result = generate_search_script(resolved, "semantic_scholar", query, params)
    if result is None:
        return {"error": "Failed to generate Semantic Scholar search script."}

    script_path, script_content = result
    return {
        "script_path": script_path,
...
@mcp.tool()
async def run_search_script(
    script_path: str,
    project_path: str,
) -> dict[str, Any]:
    """Execute a previously drafted search script.

    Run this after reviewing a script generated by
    draft_semantic_scholar_search. Results are stored in the project
    database and exported to Excel.

    Args:
        script_path: Absolute path to the search script to execute.
    ...
    return {
        ...
        "message": (
            f"Search complete. {len(results)} results stored "
            f"(search_id={search_id}). Review the Excel file at: {excel_path}"
        ),
    }
