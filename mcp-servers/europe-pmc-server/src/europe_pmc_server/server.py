...
@mcp.tool()
async def draft_europepmc_search(
    query: str,
    project_path: str,
    result_type: str = "core",
    page_size: int = 25,
    ...
    """Draft a reproducible Europe PMC search script for user review.
    ...
    result = generate_search_script(resolved, "europe_pmc", query, params)
    if result is None:
        return {"error": "Failed to generate Europe PMC search script."}
...
async def run_search_script(
    ...
    Run this after reviewing a script generated by draft_europepmc_search.
    Results are stored in the project database and exported to Excel.
